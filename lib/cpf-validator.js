...
function calculate(cpfNumber){
  let firstDigit = Number(cpfNumber[9])
  let secondDigit = Number(cpfNumber[10])
  let nineDigits = cpfNumber.slice(0, 9)
  let tenDigits = cpfNumber.slice(0, 10);

  
  function multiply(multiple, array){
   return result = array.map(Number)
   .map((number) => {
...
console.log(cpfValidator("40084948809"))
